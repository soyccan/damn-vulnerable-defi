...
contract WalletDeployer {
    ...
    function can(address u, address a) public view returns (bool) {
        assembly {
            let m := sload(0)  // this.mom (authorizer contract)
            if iszero(extcodesize(m)) {return(0, 0)}
            let p := mload(0x40)
            mstore(0x40,add(p,0x44))  // calldata.length += 68 = 4 + 32*2
            mstore(p,shl(0xe0,0x4538c4eb))  // selector = can(address,address)
            mstore(add(p,0x04),u)  // calldata[0] = u
            mstore(add(p,0x24),a)  // calldata[1] = a
            if iszero(staticcall(gas(),m,p,0x44,p,0x20)) {return(0,0)}  // call: mom.can(u, a)
